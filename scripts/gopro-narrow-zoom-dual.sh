...
# OBS sin recorte; Discord con zoom/recorte centrado ~1.4x.
...
while true; do
  ...
  log "Transmitiendo a $OUT_OBS normal y $OUT_DISCORD con zoom ~1.4x."
  ...
  ffmpeg \
    ...
    -fflags nobuffer \
    -filter_complex "[0:v]format=yuv420p,split=2[vobs][vdiscord_src];[vdiscord_src]crop=w='trunc(iw*0.72/2)*2':h='trunc(ih*0.72/2)*2':x='(iw-ow)/2':y='(ih-oh)/2',scale=1920:1080[vdiscord]" \
...
echo
echo "GoPro v3 Narrow instalada/actualizada: OBS normal + Discord zoom ~1.4x."
